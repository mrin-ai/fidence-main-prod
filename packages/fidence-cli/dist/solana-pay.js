...
function getSolanaRpcUrl() {
    return (process.env.FIDENCE_SOLANA_RPC_URL?.trim() ||
        process.env.FIDENCE_RPC_URL?.trim() ||
        "https://api.mainnet-beta.solana.com");
}
function getSolanaToken(tokenId) {
    return solanaTokenMints[tokenId.toLowerCase()] ?? null;
}
export function supportsSolanaPayment(tokenId) {
...
export async function sendLocalSolanaPayment(input) {
    ...
    const transaction = new Transaction()
        .add(createAssociatedTokenAccountIdempotentInstruction(payer.publicKey, destinationAta, recipient, mint))
