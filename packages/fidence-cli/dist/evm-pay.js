import { createPublicClient, createWalletClient, getAddress, http, parseEther, parseUnits, } from "viem";
...
function getRpcUrl(networkId) {
    ...
    if (!url) {
        throw new Error(`No RPC URL for ${networkId}. Set FIDENCE_${networkId.toUpperCase()}_RPC_URL or FIDENCE_RPC_URL.`);
    }
    return url;
}
function applyGasBuffer(estimated, cap) {
...
export async function sendLocalEvmPayment(input) {
    ...
    if (tokenId === "eth") {
        ...
        const gas = applyGasBuffer(await publicClient.estimateGas({ account: account.address, to: recipient, value }), BigInt(50_000));
