...
function loadConfig(): Config {
  try {
    return JSON.parse(readFileSync(getConfigPath(), "utf8")) as Config;
  } catch {
    return {};
  }
}
...
async function cmdSetupPoll(args: string[]) {
...
async function cmdPreflight(args: string[]) {
  const config = loadConfig();
  if (!config.apiKey) {
...
async function cmdPay(args: string[]) {
  ...
  if (intentIdx >= 0) {
    ...
    for (let i = 0; i < 120; i += 1) {
      ...
      const body = poll.data as { status?: string; intent?: unknown };
